#cs ----------------------------------------------------------------------------
 Script Function:	Base 64 encoder / Decoder
 AutoIt Version: 	3.3.10.2
 Author:			Caleb Alexander
 Credits:			Ward - Base64Encode and Base64Decode functions
#ce ----------------------------------------------------------------------------
...
Global Const 	$__Plugin_Base64_Name = 		"Base64"
Global Const 	$__Plugin_Base64_Usage = 		"Usage:<br>/base64.au3?action=encode?data=test<br>/base64.au3?action=decode?data=dGVzdA=="
Global Const 	$__Plugin_Base64_Error = 		"Improperly Formatted Parameters<br><br>" & $__Plugin_Base64_Usage
Global Const 	$__Plugin_Base64_Params[2] = 	["action", "data"]
Global 			$__Plugin_Base64_Data[2]
...
Func __Plugin_Base64_Setup()
	;
EndFunc

Func __Plugin_Base64_Main( $args )

	If StringLower($args[0]) == $__Plugin_Base64_Name & ".au3" Then
		Return $__Plugin_Base64_Usage
	EndIf

	If UBound($args) == UBound($__Plugin_Base64_Params) Then
		For $i = 0 To UBound($__Plugin_Base64_Params)-1
			If NOT StringInStr($args[$i], "=") Then Return $__Plugin_Base64_Error
			Local $aToken = __Tokenizer_TokenizeOnlyFirstToken($args[$i], "=")
			$__Plugin_Base64_Data[$i] = $aToken[1]
			If $aToken[0] <> $__Plugin_Base64_Params[$i] Then Return $__Plugin_Base64_Error
		Next
	EndIf

	Switch $__Plugin_Base64_Data[0]
		Case StringLower("encode")
			Return _Base64Encode($__Plugin_Base64_Data[1])
		Case StringLower("decode")
			Return BinaryToString( _Base64Decode($__Plugin_Base64_Data[1]) )
		Case Else
			Return $__Plugin_Base64_Error
	EndSwitch
